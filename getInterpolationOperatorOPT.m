function op = getInterpolationOperatorOPT(Pcell, Pface, Pnode, globalIndices, types, N)
...
%    isNode = ~isCell & ~isFace;
    ...
    P = sparse(numel(globalIndices), size(Pface, 2));
    
    z = find(globalIndices == 0);
    globalIndices(z) = []; isFace(z) = [];
